...
async def search_books(q: str = Query(..., min_length=1), svc: BookService = Depends(get_service)):
    return await svc.search_books(q)

@router.get("", response_model=BookListResponse)
async def list_books(page: int = 1, page_size: int = 10, svc: BookService = Depends(get_service)):
    return await svc.list_books(page, page_size)
