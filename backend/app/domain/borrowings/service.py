...
STATUS_FILTERS = {"BORROWED", "RETURNED", "OVERDUE"}
...
class BorrowingService:
    ...
    def _to_response(self, borrowing) -> BorrowingResponse:
        response = BorrowingResponse.model_validate(borrowing)
        overdue_days = self._overdue_days(borrowing)
        response.overdue_days = overdue_days
        if borrowing.status == "RETURNED":
            response.current_fine_amount = borrowing.fine_amount
        else:
            response.current_fine_amount = Decimal(overdue_days) * FINE_PER_DAY
        return response

    def _normalize_status_filter(self, status: str | None) -> str | None:
        if not status:
            return None
